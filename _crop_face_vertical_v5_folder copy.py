...
def main():
    ...
    if OVERLAY_PATH is not None:
        # Our output resolution is 1080x1920.
    ...
    if not video_files:
        print("No supported video files found in:", INPUT_FOLDER)
        return

    for video_path in video_files:
        base_name = os.path.splitext(os.path.basename(video_path))[0]
        output_path = os.path.join(OUTPUT_FOLDER, base_name + "_processed")
        print(f"\nProcessing video: {video_path}")
        process_video(video_path, output_path, net, overlay)
